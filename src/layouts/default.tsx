  ...
  return (
    <>
      ...
      <div
        className={`mb-6 grid w-full ${
          minimizeMenu
            ...
            : "lg:grid-cols-12 md:grid-cols-[repeat(26,_minmax(0,_1fr))]"
        } h-fit lg:gap-4 md:gap-2`}
      >
        <div
          className={`w-full h-full col-span-2 ${
            minimizeMenu ? "hidden" : "lg:block md:block sm:hidden xs:hidden"
          }`}
        >
          {user && subscribedTo && likedTo && log && (
            <SideMenu
              user={user}
              subscribedTo={subscribedTo}
              likedTo={likedTo}
        ...
        <div
          className={`flex flex-col items-center w-full h-full ${
            minimizeMenu
              ...
              : "lg:col-span-10 md:col-[3_/_span_28]"
          }`}
        >
          {children}
        </div>
      ...
      {subscribedTo && channel && (
        <SheetSideMenu subscribedTo={subscribedTo} channel={channel} />
      )}
    </>
  );
};
