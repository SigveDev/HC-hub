  ...
  useEffect(() => {
    if (user && user !== "error") {
      const fetchChannel = async () => {
        const res: ChannelRequest = (await getChannelByUserId(
          user.account.$id
        )) as ChannelRequest;
    ...
    <BrowserRouter>
      <ThemeProvider defaultTheme="dark" storageKey="vite-ui-theme">
        <ScrollArea className="w-full h-full pl-4 pr-4">
          {user && user !== "error" && subscribedTo && likedTo && log && (
            <DefaultLayout
              user={user}
            ...
            >
              <Routes>
                <Route
                  ...
                  element={
                    user &&
                    (user === "error" ? <Navigate to="/login" /> : <Home />)
                  }
                />
                <Route
                  path="/subscribed"
                  element={
                    ...
                    (user === "error" ? (
                      ...
                      <Subscribed subscribedTo={subscribedTo} />
                    ))
                  }
                />
                <Route
                ...
                <Route
                  ...
                  element={
                    ...
                    (user === "error" ? (
                      ...
                      <ChannelView user={user} subscribedTo={subscribedTo} />
                    ))
                  }
                />
                <Route
                ...
                <Route
                  ...
                  element={
                    user &&
                    (user === "error" ? <Navigate to="/login" /> : <Search />)
                  }
                />
                <Route
                  path="/watch?"
          ...
          <Routes>
            <Route
              path="/"
              element={user && user === "error" && <Navigate to="/login" />}
            />
            <Route
              path="/login"
              element={
                user && (user !== "error" ? <Navigate to="/" /> : <Login />)
              }
            />
