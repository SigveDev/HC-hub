...
const Watch = ({ user, subscribedTo, likedTo }: any) => {
  ...
  useEffect(() => {
    const getVideoDownloadLink = async () => {
      if (videoData) {
        const link = await getVideoDownload(videoData.video);
        setDownloadLink(link as string);
      }
    };
  ...
  return (
    <>
      <div className="flex flex-col items-center justify-center w-full mb-4 h-fit">
        <div
          className={`grid w-full lg:grid-cols-3 md:grid-cols-1 sm:grid-cols-1 xs:grid-cols-1 h-fit max-w-[1500px] mt-6 gap-4`}
        >
          <div className="w-full lg:col-span-2 md:col-span-1 sm:col-span-1 xs:col-span-1 h-fit">
            ...
            <div className="flex flex-col w-full h-full mt-4">
              ...
              <div className="flex flex-row items-center justify-start w-full h-12 mt-4 mb-1">
                ...
                <div className="flex flex-col h-full pl-2 pr-6 w-fit">
                  <a
                    href={"/channel/" + thisChannel?.$id}
                    className="mt-0 ml-2 text-md"
                  >
                    {thisChannel?.username}
                  </a>
                ...
                {thisChannel ? (
                  myChannel ? (
                    <Button
                      className="h-10 pl-4 pr-4 mt-auto mb-auto text-sm rounded-full w-fit"
                      variant="default"
                      asChild
                    >
                      <a href="/channel/edit">Edit</a>
                    </Button>
                  ) : isSubscribed ? (
                    <Button
                      className="h-10 pl-4 pr-4 mt-auto mb-auto text-sm rounded-full w-fit"
            ...
            {videoData ? (
              <div
                className={`flex flex-col w-full p-2 mt-4 h-fit rounded-xl ${
              ...
              >
                ...
                <div className="w-full text-sm text-pretty h-fit">
            ...
            <div className="flex flex-col w-full gap-2 mt-2 h-fit">
              {relatedVideos &&
                relatedVideos.map((video, index) => {
                  ...
                  return <VideoView3 {...video} key={index} />;
                })}
            </div>
          </div>
        </div>
