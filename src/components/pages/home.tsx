...
const Home = () => {
  ...
  const activeCat =
    new URLSearchParams(window.location.search).get("cat")?.toString() || "";
  const lodingAmount = [1, 2, 3, 4, 5, 6];

  useEffect(() => {
    const getVideosAsync = async () => {
      const videos: VideoRequest = (await getVideosByCategory(
        activeCat
      )) as VideoRequest;
      ...
      <div className="grid w-full gap-4 mt-2 sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-5 h-fit">
        {videos != undefined
          ? videos.map((video, index) => {
              return <VideoView {...video} key={index} />;
            })
          : lodingAmount.map((_, index) => {
              return (
                <div
                  className="flex flex-col w-full aspect-[6/5] gap-2"
                  key={index}
                >
