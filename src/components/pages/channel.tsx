  ...
  return (
    <>
        ...
        <div className="flex flex-col w-full h-full gap-2 pl-4">
          {thisChannel ? (
            <h1 className="text-2xl font-bold">
              {thisChannel && thisChannel.username}
            </h1>
          ) : (
          ...
          {thisChannel ? (
            <p className="text-sm">
              {thisChannel && thisChannel.Subscribers.length} Subscribers
            </p>
          ) : (
      ...
      <div className="grid max-w-[1000px] gap-2 mt-4 xs:grid-cols-1 sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-4">
        {thisChannel
          ? thisChannel.Videos.map((video: any, index: number) => (
              <VideoView4 {...video} key={index} />
            ))
          : lodingAmount.map((_, index) => {
              return (
                <div
                  className="flex flex-col w-full gap-1 border-none aspect-square"
                  key={index}
                >
