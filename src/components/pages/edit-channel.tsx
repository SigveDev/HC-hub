...
const EditChannel = ({ channel }: any) => {
  ...
  useEffect(() => {
    const fetchChannel = async () => {
      const currentChannel: Channel = (await getChannelById(
        channel.$id
      )) as Channel;
      setUsername(currentChannel.username);

      if (currentChannel) {
        const pfpData: ImageData = (await getPFP(
          currentChannel.pfp
        )) as ImageData;
        setPrevFile(pfpData);
      }
  ...
  const handleCreateChannel = async () => {
    if (username.length < 64 && username.length > 4) {
      if (file) {
        setLoader(true);
        if (file !== undefined) {
          const pfp: any = (await updatePFP(
            channel.$id,
            channel.pfp,
            file
          )) as any;
          const updatedChannel: Channel = (await updateChannel(
            channel.$id,
            username,
            pfp.$id
          )) as Channel;
