            ...
            {!minimizeMenu && <div className="flex flex-col w-full h-full gap-4 pt-1">
                ...
                <div className="flex flex-col items-center w-full gap-2 h-fit">
                    ...
                    {subscribedTo && subscribedTo[0].Channels.map((channel: any, index: number) => {
                        ...
                        return (
                            <a href={"/channel/" + channel.$id} className={`flex flex-row items-center justify-start h-10 gap-2 p-2 md:rounded-full lg:rounded-lg ${minimizeMenu ? "lg:w-fit" : "lg:w-full"} ${page === ("channel/" + channel.$id) && (theme === "dark" ? "bg-slate-800" : "bg-slate-200")} ${theme === "dark" ? "hover:bg-slate-800" : "hover:bg-slate-200"} md:w-fit md:bg-transparent`} key={index}>{localImage && <img src={localImage.toString()} className='h-full rounded-full aspect-square' />}<span className={`w-full h-full break-all ${minimizeMenu ? "lg:hidden" : "lg:block"} md:hidden sm:hidden`}>{channel.username}</span></a>
                        );
                    })}
                </div>
            </div>}
