    ...
    <div className="flex flex-row items-center justify-between w-full h-12 mt-4">
      <div className="flex flex-row items-center justify-center gap-2 h-fit w-fit">
        <button
          ...
          className={
            minimizeMenu ? "block" : "lg:hidden md:hidden sm:block xs:block"
          }
        >
          <Menu />
        </button>
        <a href="/">
          <h1 className="flex items-center justify-center h-full text-xl font-semibold w-fit">
            HC Hub
          </h1>
        </a>
      </div>
      <form
        className="flex flex-row items-center justify-center w-1/3 h-full"
        onSubmit={(e) => handleSearch(e)}
      >
        <Input
          type="search"
          placeholder="Search"
          className="w-5/6 pl-4 rounded-r-none rounded-l-3xl focus-visible:ring-transparent"
          value={searchValue}
          onChange={(e) => setSearchValue(e.target.value.toString())}
        />
        <Button
          type="submit"
          className="grid w-1/6 rounded-l-none rounded-r-3xl place-items-center"
          variant="secondary"
        >
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent>
          ...
          <DropdownMenuItem className="cursor-pointer">
            <a href={"/channel/" + (channel !== undefined && channel.$id)}>
              Channel
            </a>
          </DropdownMenuItem>
          <DropdownMenuItem className="cursor-pointer">
            <a href="https://studio.hub.hcklikk.com">Studio</a>
          </DropdownMenuItem>
          <DropdownMenuItem className="cursor-pointer">
            <a href={settingLink}>Settings</a>
          </DropdownMenuItem>
          <DropdownMenuSeparator />
