            ...
            <SheetContent side="left" className='w-fit'>
                ...
                <div className="flex flex-col w-full h-full gap-4 pt-1">
                    ...
                    <div className="flex flex-col w-full gap-2 h-fit">
                        ...
                        {channel !== undefined ? <a onClick={handleToggleMenu} href={"/channel/" + channel?.$id} className={`flex flex-row items-center justify-start h-10 gap-2 p-2 rounded-lg lg:w-full ${page === ("channel/" + channel.$id) && (theme === "dark" ? "bg-slate-800" : "bg-slate-200")} ${theme === "dark" ? "hover:bg-slate-800" : "hover:bg-slate-200"}`}><SquareUserRound /><span className={`lg:block`}>Your Channel</span></a> : <a onClick={handleToggleMenu} href="/channel/create" className={`flex flex-row items-center justify-start h-10 gap-2 p-2 rounded-lg lg:w-full ${page === "channel/create" && (theme === "dark" ? "bg-slate-800" : "bg-slate-200")} ${theme === "dark" ? "hover:bg-slate-800" : "hover:bg-slate-200"}`}><SquareUserRound /><span className={`lg:block`}>Create Channel</span></a>}
                    ...
                    <div className="flex flex-col w-full gap-2 h-fit">
                        ...
                        {subscribedTo && subscribedTo[0].Channels.map((channel: any, index: number) => {
                            ...
                            return (
                                <a onClick={handleToggleMenu} href={"/channel/" + channel.$id} className={`flex flex-row items-center justify-start h-10 gap-2 p-2 rounded-lg w-full ${page === ("channel/" + channel.$id) && (theme === "dark" ? "bg-slate-800" : "bg-slate-200")} ${theme === "dark" ? "hover:bg-slate-800" : "hover:bg-slate-200"}`} key={index}>{localImage && <img src={localImage.toString()} className='h-full rounded-full aspect-square' />}<span className={`w-full h-full break-all lg:block`}>{channel.username}</span></a>
                            );
                        })}
                    </div>
                </div>
