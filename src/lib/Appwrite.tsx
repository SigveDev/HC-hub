...
client
    .setEndpoint(import.meta.env.VITE_PROJECT_ENDPOINT || 'https://cloud.appwrite.io/v1')
...
export const getPFP = (pfpId: string) => {
    try {
        const pfp = storage.getFileView(import.meta.env.VITE_PFP_BUCKET_ID || '', pfpId);
        return pfp;
    } catch (error) {
        return error;
    }
...
export const getImage = (imageId: string) => {
    try {
        const image = storage.getFileView(import.meta.env.VITE_IMAGES_BUCKET_ID || '', imageId);
        return image;
    } catch (error) {
        return error;
    }
...
export const getVideo = (videoId: string) => {
    try {
        const video = storage.getFileView(import.meta.env.VITE_VIDEOS_BUCKET_ID || '', videoId);
        return video;
    } catch (error) {
        return error;
    }
